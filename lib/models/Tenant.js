...
module.exports = class Tenant {
    ...
    static async insert({ name, email, password, propertyAddress, ownerId, monthlyCost }) {
        const { rows } = await pool.query(
            `INSERT INTO tenants (name, email, password, property_address, owner_id, monthly_cost) VALUES ($1, $2, $3, $4, $5, $6) RETURNING *`,
            [name, email, password, propertyAddress, ownerId, monthlyCost]
        );
    ...
    static async update(id, { name, email, password, propertyAddress, ownerId, monthlyCost }) {
        const { rows } = await pool.query(
            `UPDATE tenants
            SET name=$1,
            email=$2,
            ...
            WHERE id=$7
            RETURNING *`,
            [name, email, password, propertyAddress, ownerId, monthlyCost, id]
        );
